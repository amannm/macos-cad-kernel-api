...
struct occ_shape_t {
  occ_header_t h;
  TopoDS_Shape shape;
  explicit occ_shape_t(const TopoDS_Shape& s) : h(OCC_HANDLE_SHAPE), shape(s) {}
};
...
{
  occ_status_t s = check_kind(p, kind);
  if (s != OCC_OK) {
    return s;
  }
  *out = p;
...
template <class T>
occ_status_t get_handle(T* p, occ_handle_kind_t kind, T** out)
{
  occ_status_t s = check_kind(p, kind);
  if (s != OCC_OK) {
    return s;
...
{
  return gp_Ax1(point(a.origin), direction(a.direction));
}

gp_Ax2 axis2(occ_axis2_t a)
{
  return gp_Ax2(point(a.origin), direction(a.z_direction), direction(a.x_direction));
}

gp_Ax3 axis3(occ_axis2_t a)
{
  return gp_Ax3(axis2(a));
}
...
occ_status_t expect_shape_kind(const occ_shape_t* s, TopAbs_ShapeEnum t)
{
  if (s->shape.IsNull()) {
...
NCollection_List<TopoDS_Shape> shape_list(const occ_shape_t* const* shapes, size_t count, occ_status_t& status)
{
...
std::vector<TopoDS_Shape> collect_subshapes(const TopoDS_Shape& shape, TopAbs_ShapeEnum type, occ_traversal_t traversal)
{
...
occ_status_t read_brep_buffer(const void* data, size_t size, occ_shape_t** out)
{
  ...
  *out = nullptr;
  ...
  std::string payload(bytes, bytes + size);
...
occ_status_t write_brep_buffer(const TopoDS_Shape& shape, occ_brep_format_t format, void** out_data, size_t* out_size)
{
...
void destroy_handle(void* handle)
{
  ...
  h->magic = OCC_DEAD;
  switch (h->kind) {
    case OCC_HANDLE_CURVE2D: delete static_cast<occ_curve2d_t*>(handle); return;
    case OCC_HANDLE_CURVE3D: delete static_cast<occ_curve3d_t*>(handle); return;
    case OCC_HANDLE_SURFACE: delete static_cast<occ_surface_t*>(handle); return;
    case OCC_HANDLE_SHAPE: delete static_cast<occ_shape_t*>(handle); return;
    case OCC_HANDLE_MESH: delete static_cast<occ_mesh_t*>(handle); return;
...
OCC_C_API occ_status_t OCC_C_CALL occ_context_set_log_callback(occ_context_t* context, occ_log_callback_t callback, void* user_data)
{
  return guard(context, [&]() {
    occ_status_t s = check_context(context);
    if (s != OCC_OK) {
      return s;
    }
...
OCC_C_API occ_status_t OCC_C_CALL occ_context_last_diagnostic_at(const occ_context_t* context, size_t index, occ_log_level_t* out_level, const char** out_message_utf8)
{
...
OCC_C_API occ_status_t OCC_C_CALL occ_curve3d_line(occ_context_t* context, occ_vec3_t p, occ_vec3_t d, occ_curve3d_t** out_curve)
{
  return guard(context, [&]() {
    if (check_context(context) != OCC_OK || out_curve == nullptr) {
      return out_curve == nullptr ? OCC_NULL_ARGUMENT : OCC_NULL_ARGUMENT;
    }
    *out_curve = nullptr;
    occ::handle<Geom_Curve> c = new Geom_Line(point(p), direction(d));
    return put_curve3d(out_curve, c);
  });
}

OCC_C_API occ_status_t OCC_C_CALL occ_curve3d_circle(occ_context_t* context, occ_axis2_t frame, double radius, occ_curve3d_t** out_curve)
{
  return guard(context, [&]() {
    if (check_context(context) != OCC_OK || out_curve == nullptr) {
      return OCC_NULL_ARGUMENT;
    ...
    *out_curve = nullptr;
    occ::handle<Geom_Curve> c = new Geom_Circle(axis2(frame), radius);
    return put_curve3d(out_curve, c);
  });
}
...
OCC_C_API occ_status_t OCC_C_CALL occ_curve3d_type(const occ_curve3d_t* curve, occ_curve_type_t* out_type)
{
  return guard(nullptr, [&]() {
    ...
    occ_status_t s = require_curve3d(curve, &c);
...
OCC_C_API occ_status_t OCC_C_CALL occ_curve3d_eval(const occ_curve3d_t* curve, double parameter, occ_vec3_t* out_point)
{
  return guard(nullptr, [&]() {
    const occ_curve3d_t* c = nullptr;
    occ_status_t s = require_curve3d(curve, &c);
...
OCC_C_API occ_status_t OCC_C_CALL occ_curve3d_eval_d1(const occ_curve3d_t* curve, double parameter, occ_vec3_t* out_point, occ_vec3_t* out_derivative)
{
  return guard(nullptr, [&]() {
    const occ_curve3d_t* c = nullptr;
    occ_status_t s = require_curve3d(curve, &c);
...
OCC_C_API occ_status_t OCC_C_CALL occ_surface_plane(occ_context_t* context, occ_axis2_t frame, occ_surface_t** out_surface)
{
  return guard(context, [&]() {
    ...
    *out_surface = nullptr;
    occ::handle<Geom_Surface> s = new Geom_Plane(axis3(frame));
    return put_surface(out_surface, s);
  });
}

OCC_C_API occ_status_t OCC_C_CALL occ_surface_cylinder(occ_context_t* context, occ_axis2_t frame, double radius, occ_surface_t** out_surface)
{
  return guard(context, [&]() {
    if (context == nullptr || out_surface == nullptr) {
      return OCC_NULL_ARGUMENT;
    ...
    *out_surface = nullptr;
    occ::handle<Geom_Surface> s = new Geom_CylindricalSurface(axis3(frame), radius);
    return put_surface(out_surface, s);
  });
}

OCC_C_API occ_status_t OCC_C_CALL occ_surface_sphere(occ_context_t* context, occ_axis2_t frame, double radius, occ_surface_t** out_surface)
{
  return guard(context, [&]() {
    if (context == nullptr || out_surface == nullptr) {
      return OCC_NULL_ARGUMENT;
    ...
    *out_surface = nullptr;
    occ::handle<Geom_Surface> s = new Geom_SphericalSurface(axis3(frame), radius);
    return put_surface(out_surface, s);
  });
}
...
OCC_C_API occ_status_t OCC_C_CALL occ_surface_type(const occ_surface_t* surface, occ_surface_type_t* out_type)
{
  return guard(nullptr, [&]() {
    ...
    occ_status_t s = require_surface(surface, &srf);
    if (s != OCC_OK || out_type == nullptr) {
      return s != OCC_OK ? s : OCC_NULL_ARGUMENT;
    }
...
OCC_C_API occ_status_t OCC_C_CALL occ_surface_bounds(const occ_surface_t* surface, double* out_umin, double* out_umax, double* out_vmin, double* out_vmax)
{
  return guard(nullptr, [&]() {
    const occ_surface_t* srf = nullptr;
    occ_status_t s = require_surface(surface, &srf);
...
OCC_C_API occ_status_t OCC_C_CALL occ_surface_eval(const occ_surface_t* surface, double u, double v, occ_vec3_t* out_point)
{
  return guard(nullptr, [&]() {
    const occ_surface_t* srf = nullptr;
    occ_status_t s = require_surface(surface, &srf);
...
OCC_C_API occ_status_t OCC_C_CALL occ_surface_eval_d1(const occ_surface_t* surface, double u, double v, occ_vec3_t* out_point, occ_vec3_t* out_du, occ_vec3_t* out_dv)
{
  return guard(nullptr, [&]() {
    const occ_surface_t* srf = nullptr;
    occ_status_t s = require_surface(surface, &srf);
...
OCC_C_API occ_status_t OCC_C_CALL occ_project_point_curve3d(occ_context_t* context, occ_vec3_t p, const occ_curve3d_t* curve, double* out_parameter, occ_vec3_t* out_nearest, double* out_distance)
{
  return guard(context, [&]() {
    const occ_curve3d_t* c = nullptr;
    occ_status_t s = require_curve3d(curve, &c);
    if (context == nullptr || s != OCC_OK || out_parameter == nullptr || out_nearest == nullptr || out_distance == nullptr) {
      return context == nullptr ? OCC_NULL_ARGUMENT : (s != OCC_OK ? s : OCC_NULL_ARGUMENT);
    }
    GeomAPI_ProjectPointOnCurve projector(point(p), c->curve);
...
OCC_C_API occ_status_t OCC_C_CALL occ_project_point_surface(occ_context_t* context, occ_vec3_t p, const occ_surface_t* surface, double* out_u, double* out_v, occ_vec3_t* out_nearest, double* out_distance)
{
  return guard(context, [&]() {
    const occ_surface_t* srf = nullptr;
    occ_status_t s = require_surface(surface, &srf);
    if (context == nullptr || s != OCC_OK || out_u == nullptr || out_v == nullptr || out_nearest == nullptr || out_distance == nullptr) {
      return context == nullptr ? OCC_NULL_ARGUMENT : (s != OCC_OK ? s : OCC_NULL_ARGUMENT);
    }
    GeomAPI_ProjectPointOnSurf projector(point(p), srf->surface);
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_type(const occ_shape_t* shape, occ_shape_type_t* out_type)
{
  return guard(nullptr, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_set_location(occ_context_t* context, const occ_shape_t* shape, occ_mat4_t location, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
    ...
    *out_shape = nullptr;
    TopoDS_Shape located = s->shape.Located(TopLoc_Location(trsf(location)));
    return put_shape(out_shape, located);
  });
}

OCC_C_API occ_status_t OCC_C_CALL occ_shape_reversed(occ_context_t* context, const occ_shape_t* shape, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_transformed(occ_context_t* context, const occ_shape_t* shape, occ_mat4_t transform, int32_t copy_geometry, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_bbox(occ_context_t* context, const occ_shape_t* shape, int32_t use_triangulation, occ_bbox_t* out_bbox)
{
  return guard(context, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_tolerance(const occ_shape_t* shape, double* out_tolerance)
{
  return guard(nullptr, [&]() {
    ...
    switch (s->shape.ShapeType()) {
      case TopAbs_VERTEX: *out_tolerance = BRep_Tool::Tolerance(TopoDS::Vertex(s->shape)); return OCC_OK;
      case TopAbs_EDGE: *out_tolerance = BRep_Tool::Tolerance(TopoDS::Edge(s->shape)); return OCC_OK;
      case TopAbs_FACE: *out_tolerance = BRep_Tool::Tolerance(TopoDS::Face(s->shape)); return OCC_OK;
      default:
        *out_tolerance = std::max({BRep_Tool::MaxTolerance(s->shape, TopAbs_VERTEX), BRep_Tool::MaxTolerance(s->shape, TopAbs_EDGE), BRep_Tool::MaxTolerance(s->shape, TopAbs_FACE)});
        return OCC_OK;
    }
  });
}

OCC_C_API occ_status_t OCC_C_CALL occ_shape_count_subshapes(occ_context_t* context, const occ_shape_t* shape, occ_shape_type_t type, occ_traversal_t traversal, size_t* out_count)
{
  return guard(context, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_subshape_at(occ_context_t* context, const occ_shape_t* shape, occ_shape_type_t type, occ_traversal_t traversal, size_t index, occ_shape_t** out_subshape)
{
  return guard(context, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_visit_subshapes(occ_context_t* context, const occ_shape_t* shape, occ_shape_type_t type, occ_traversal_t traversal, occ_shape_visit_fn visitor, void* user_data)
{
  return guard(context, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
    ...
    for (const TopoDS_Shape& current : found) {
      occ_shape_t* wrapped = new occ_shape_t(current);
      occ_status_t visit = visitor(wrapped, shape_type(current.ShapeType()), user_data);
      occ_release(wrapped);
      if (visit != OCC_OK) {
        return visit;
      }
...
OCC_C_API occ_status_t OCC_C_CALL occ_edge_curve3d(occ_context_t* context, const occ_shape_t* edge, occ_curve3d_t** out_curve, double* out_first, double* out_last)
{
  return guard(context, [&]() {
    ...
    *out_curve = nullptr;
    ...
    occ::handle<Geom_Curve> c = BRep_Tool::Curve(TopoDS::Edge(e->shape), *out_first, *out_last);
    return put_curve3d(out_curve, c);
  });
}

OCC_C_API occ_status_t OCC_C_CALL occ_edge_curve2d_on_face(occ_context_t* context, const occ_shape_t* edge, const occ_shape_t* face, occ_curve2d_t** out_curve, double* out_first, double* out_last)
{
  return guard(context, [&]() {
    const occ_shape_t* e = nullptr;
    ...
    *out_curve = nullptr;
    ...
    occ::handle<Geom2d_Curve> c = BRep_Tool::CurveOnSurface(TopoDS::Edge(e->shape), TopoDS::Face(f->shape), *out_first, *out_last);
...
OCC_C_API occ_status_t OCC_C_CALL occ_curve2d_type(const occ_curve2d_t* curve, occ_curve_type_t* out_type)
{
  return guard(nullptr, [&]() {
    ...
    occ_status_t s = require_curve2d(curve, &c);
    if (s != OCC_OK || out_type == nullptr) {
      return s != OCC_OK ? s : OCC_NULL_ARGUMENT;
    }
...
OCC_C_API occ_status_t OCC_C_CALL occ_curve2d_eval(const occ_curve2d_t* curve, double parameter, occ_vec2_t* out_point)
{
  return guard(nullptr, [&]() {
    const occ_curve2d_t* c = nullptr;
    occ_status_t s = require_curve2d(curve, &c);
...
OCC_C_API occ_status_t OCC_C_CALL occ_curve2d_eval_d1(const occ_curve2d_t* curve, double parameter, occ_vec2_t* out_point, occ_vec2_t* out_derivative)
{
  return guard(nullptr, [&]() {
    const occ_curve2d_t* c = nullptr;
    occ_status_t s = require_curve2d(curve, &c);
...
OCC_C_API occ_status_t OCC_C_CALL occ_face_surface(occ_context_t* context, const occ_shape_t* face, occ_surface_t** out_surface)
{
  return guard(context, [&]() {
    const occ_shape_t* f = nullptr;
...
OCC_C_API occ_status_t OCC_C_CALL occ_face_uv_bounds(occ_context_t* context, const occ_shape_t* face, double* out_umin, double* out_umax, double* out_vmin, double* out_vmax)
{
  return guard(context, [&]() {
    const occ_shape_t* f = nullptr;
    occ_status_t s = require_shape(face, &f);
    if (context == nullptr || s != OCC_OK || out_umin == nullptr || out_umax == nullptr || out_vmin == nullptr || out_vmax == nullptr) {
      return context == nullptr ? OCC_NULL_ARGUMENT : (s != OCC_OK ? s : OCC_NULL_ARGUMENT);
    }
    s = expect_shape_kind(f, TopAbs_FACE);
    if (s != OCC_OK) {
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_edge_points(occ_context_t* context, occ_vec3_t start, occ_vec3_t end, occ_shape_t** out_edge)
{
  return guard(context, [&]() {
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_edge_curve3d(occ_context_t* context, const occ_curve3d_t* curve, double first, double last, occ_shape_t** out_edge)
{
  return guard(context, [&]() {
    const occ_curve3d_t* c = nullptr;
    occ_status_t s = require_curve3d(curve, &c);
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_wire(occ_context_t* context, const occ_shape_t* const* edges, size_t edge_count, occ_shape_t** out_wire)
{
  return guard(context, [&]() {
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_face_wire(occ_context_t* context, const occ_shape_t* wire, int32_t force_planar, occ_shape_t** out_face)
{
  return guard(context, [&]() {
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_face_surface_bounds(occ_context_t* context, const occ_surface_t* surface, double umin, double umax, double vmin, double vmax, double tolerance, occ_shape_t** out_face)
{
  return guard(context, [&]() {
    const occ_surface_t* srf = nullptr;
    occ_status_t s = require_surface(surface, &srf);
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_shell(occ_context_t* context, const occ_shape_t* const* faces, size_t face_count, occ_shape_t** out_shell)
{
  return guard(context, [&]() {
    if (context == nullptr || out_shell == nullptr || face_count == 0 || faces == nullptr) {
      return context == nullptr || out_shell == nullptr || faces == nullptr ? OCC_NULL_ARGUMENT : OCC_INVALID_ARGUMENT;
    }
    *out_shell = nullptr;
    BRep_Builder builder;
    TopoDS_Shell shell;
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_solid(occ_context_t* context, const occ_shape_t* shell, occ_shape_t** out_solid)
{
  return guard(context, [&]() {
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_compound(occ_context_t* context, const occ_shape_t* const* shapes, size_t shape_count, occ_shape_t** out_compound)
{
  return guard(context, [&]() {
    ...
    *out_compound = nullptr;
    ...
    for (size_t i = 0; i < shape_count; ++i) {
      const occ_shape_t* s = nullptr;
      occ_status_t status = require_shape(shapes[i], &s);
      if (status != OCC_OK) {
        return status;
      }
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_box(occ_context_t* context, double dx, double dy, double dz, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_box_between(occ_context_t* context, occ_vec3_t min_corner, occ_vec3_t max_corner, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
    if (context == nullptr || out_shape == nullptr) {
      return OCC_NULL_ARGUMENT;
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_sphere(occ_context_t* context, occ_vec3_t center, double radius, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
    if (context == nullptr || out_shape == nullptr) {
      return OCC_NULL_ARGUMENT;
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_cylinder(occ_context_t* context, occ_axis2_t axis, double radius, double height, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
    if (context == nullptr || out_shape == nullptr) {
      return OCC_NULL_ARGUMENT;
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_cone(occ_context_t* context, occ_axis2_t axis, double radius1, double radius2, double height, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
    if (context == nullptr || out_shape == nullptr) {
      return OCC_NULL_ARGUMENT;
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_torus(occ_context_t* context, occ_axis2_t axis, double major_radius, double minor_radius, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
    if (context == nullptr || out_shape == nullptr) {
      return OCC_NULL_ARGUMENT;
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_prism(occ_context_t* context, const occ_shape_t* profile, occ_vec3_t v, int32_t copy_profile, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
    const occ_shape_t* p = nullptr;
    occ_status_t s = require_shape(profile, &p);
...
OCC_C_API occ_status_t OCC_C_CALL occ_make_revolution(occ_context_t* context, const occ_shape_t* profile, occ_axis1_t ax, double angle_radians, int32_t copy_profile, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
    const occ_shape_t* p = nullptr;
    occ_status_t s = require_shape(profile, &p);
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_check(occ_context_t* context, const occ_shape_t* shape, occ_shape_validity_t* out_validity)
{
  return guard(context, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_fix_basic(occ_context_t* context, const occ_shape_t* shape, occ_shape_t** out_fixed_shape)
{
  return guard(context, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_linear_properties(occ_context_t* context, const occ_shape_t* shape, occ_mass_properties_t* out_properties)
{
  return guard(context, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_surface_properties(occ_context_t* context, const occ_shape_t* shape, occ_mass_properties_t* out_properties)
{
  return guard(context, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_volume_properties(occ_context_t* context, const occ_shape_t* shape, occ_mass_properties_t* out_properties)
{
  return guard(context, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_distance(occ_context_t* context, const occ_shape_t* a, const occ_shape_t* b, double* out_distance, occ_vec3_t* out_nearest_on_a, occ_vec3_t* out_nearest_on_b)
{
  return guard(context, [&]() {
    ...
    if (context == nullptr || status != OCC_OK || out_distance == nullptr || out_nearest_on_a == nullptr || out_nearest_on_b == nullptr) {
      return context == nullptr ? OCC_NULL_ARGUMENT : (status != OCC_OK ? status : OCC_NULL_ARGUMENT);
    }
    BRepExtrema_DistShapeShape dist(sa->shape, sb->shape);
...
OCC_C_API occ_status_t OCC_C_CALL occ_boolean_apply(occ_context_t* context, occ_boolean_operation_t operation, const occ_shape_t* a, const occ_shape_t* b, const occ_boolean_options_t* options_or_null, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
    const occ_shape_t* sa = nullptr;
    const occ_shape_t* sb = nullptr;
...
OCC_C_API occ_status_t OCC_C_CALL occ_boolean_fuse_many(occ_context_t* context, const occ_shape_t* const* shapes, size_t shape_count, const occ_boolean_options_t* options_or_null, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
...
OCC_C_API occ_status_t OCC_C_CALL occ_boolean_cut_many(occ_context_t* context, const occ_shape_t* base, const occ_shape_t* const* tools_in, size_t tool_count, const occ_boolean_options_t* options_or_null, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
    ...
    if (context == nullptr || status != OCC_OK || out_shape == nullptr || tool_count == 0 || tools_in == nullptr) {
      return context == nullptr || out_shape == nullptr || tools_in == nullptr ? OCC_NULL_ARGUMENT : (status != OCC_OK ? status : OCC_INVALID_ARGUMENT);
    }
    *out_shape = nullptr;
    NCollection_List<TopoDS_Shape> arguments;
    NCollection_List<TopoDS_Shape> tools;
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_read_brep_file(occ_context_t* context, const char* path_utf8, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
    ...
    *out_shape = nullptr;
    std::string bytes;
    occ_status_t s = read_file_bytes(path_utf8, bytes);
    if (s != OCC_OK) {
      return s;
    }
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_write_brep_file(occ_context_t* context, const occ_shape_t* shape, const char* path_utf8, occ_brep_format_t format)
{
  return guard(context, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_read_brep_memory(occ_context_t* context, const void* data, size_t size, occ_shape_t** out_shape)
{
  return guard(context, [&]() {
    if (context == nullptr) {
      return OCC_NULL_ARGUMENT;
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_write_brep_memory(occ_context_t* context, const occ_shape_t* shape, occ_brep_format_t format, void** out_data, size_t* out_size)
{
  return guard(context, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
...
OCC_C_API occ_status_t OCC_C_CALL occ_shape_triangulate(occ_context_t* context, const occ_shape_t* shape, const occ_mesh_options_t* options_or_null, occ_shape_t** out_shape_with_triangulation)
{
  return guard(context, [&]() {
    const occ_shape_t* s = nullptr;
    occ_status_t status = require_shape(shape, &s);
    ...
    *out_shape_with_triangulation = nullptr;
    ...
    TopoDS_Shape copy = s->shape;
    BRepMesh_IncrementalMesh mesh(copy, o.linear_deflection, o.relative_deflection != 0, o.angular_deflection, o.run_parallel != 0);
    if (!mesh.IsDone()) {
      return OCC_EMPTY_RESULT;
    }
...
OCC_C_API occ_status_t OCC_C_CALL occ_face_triangulation(occ_context_t* context, const occ_shape_t* face, int32_t apply_face_location, occ_mesh_t** out_mesh)
{
  return guard(context, [&]() {
    const occ_shape_t* f = nullptr;
    ...
    *out_mesh = nullptr;
    ...
    occ::handle<Poly_Triangulation> triangulation = BRep_Tool::Triangulation(TopoDS::Face(f->shape), location);
    if (triangulation.IsNull() || triangulation->NbNodes() == 0 || triangulation->NbTriangles() == 0) {
      return OCC_EMPTY_RESULT;
    }
    occ_mesh_t* mesh = new occ_mesh_t;
...
OCC_C_API occ_status_t OCC_C_CALL occ_mesh_view(const occ_mesh_t* mesh, occ_mesh_view_t* out_view)
{
  return guard(nullptr, [&]() {
    ...
    occ_status_t status = require_mesh(mesh, &m);
